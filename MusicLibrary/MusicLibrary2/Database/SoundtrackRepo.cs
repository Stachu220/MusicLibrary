...
namespace MusicLibrary2.Database
{
    internal class SoundtrackRepo
    {
        ...
        public static List<Soundtrack> GetAllSoundtracks()
        {
        ...
        public static void RemoveSoundtrack(Soundtrack soundtrack)
        {
            using (var connection = new SQLiteConnection(Connection))
            {
                connection.Open();
                var query = "DELETE FROM Soundtrack WHERE Filename = @Filename";
                using (var command = new SQLiteCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@Filename", soundtrack.Filename);
                    command.ExecuteNonQuery();
